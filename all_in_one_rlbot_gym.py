...
def ensure_packages(packages: list[str], *, upgrade_tools: bool = False, strict: bool = True) -> None:
    """Install missing packages. In non-strict mode, log and continue on failures."""
    if upgrade_tools:
        for tool in ("pip", "setuptools", "wheel"):
            try:
                LOG.info("Upgrading packaging tool: %s", tool)
                run_python_module("pip", ["install", "--upgrade", tool])
            except Exception as exc:
                msg = f"Could not upgrade {tool}: {exc}"
                if strict:
    ...
    for pkg in packages:
        ...
        if not _module_exists(mod):
            try:
                LOG.info("Installing missing package: %s", pkg)
                run_python_module("pip", ["install", pkg])
            except Exception as exc:
                msg = f"Could not install required package {pkg}: {exc}"
                if strict:
                    raise RuntimeError(msg) from exc
                LOG.warning(msg)
...
def doctor() -> None:
    ...
    ensure_packages(DEFAULT_PACKAGES, upgrade_tools=False, strict=False)
    write_rlbot_files("out/ppo_rlgym.zip")
    create_click_to_train_files()

    try:
        _ = build_default_env()
        LOG.info("Environment smoke-check passed.")
    except Exception as exc:
        LOG.warning("Environment smoke-check skipped/failed: %s", exc)

    missing = [pkg for pkg in DEFAULT_PACKAGES if not _module_exists(pkg.replace('-', '_'))]
    if missing:
        LOG.warning("Preflight completed with missing packages: %s", ", ".join(missing))
        LOG.warning("If downloads are blocked, connect to the internet and run: python all_in_one_rlbot_gym.py install")
    else:
